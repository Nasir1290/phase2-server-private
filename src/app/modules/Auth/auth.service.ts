...
const loginUser = async (payload: { email: string; password: string }) => {
  ...
  if (!payload.password || !userData?.password) {
    throw new ApiError(httpStatus.NOT_FOUND, "Password is required");
  }
...
<body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f6f9fc; margin: 0; padding: 0; line-height: 1.6;">
  ...
  return { message: "OTP sent successfully.Verify your email address." };
};
...
const getMyProfile = async (userToken: string) => {
  ...
  const result = await prisma.$transaction(async (TransactionClient) => {
    ...
    const isCarOwner = await prisma.user.findUnique({
      where: {
        id: userProfile.id,
        Car: {
          some: {},
    ...
    return { ...userProfile, isCarOwner: isCarOwner ? true : false };
  });
